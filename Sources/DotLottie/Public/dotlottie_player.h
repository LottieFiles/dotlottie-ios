...
/**
 * Load a single animation (and optionally a theme and state machine) from a `.lottie`
 * archive **without** retaining the archive in memory.
 *
 * The archive is dropped after extraction, so `dotlottie_get_manifest`,
 * `dotlottie_load_animation` and `dotlottie_set_theme` (by id) are not available afterwards.
 *
 * - `animation_id`: required; pass an empty string to select the active/initial animation.
 * - `theme_id`: nullable; applied when non-null.
 * - `state_machine_id`: nullable; when non-null and `sm_out` is non-null, the extracted state
 *   machine is started and the engine is written to `*sm_out`. The caller owns it and must
 *   release it with `dotlottie_state_machine_release()` before destroying the runtime.
 */
enum dotlottieDotLottieResult dotlottie_load_dotlottie_data_no_cache(struct dotlottiePlayer *ptr,
                                                                     const char *file_data,
                                                                     uintptr_t file_size,
                                                                     const char *animation_id,
                                                                     const char *theme_id,
                                                                     const char *state_machine_id,
                                                                     struct dotlottieDotLottieStateMachine **sm_out);
